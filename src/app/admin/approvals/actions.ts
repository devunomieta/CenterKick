...
// 3. New Account Registration Approvals
export async function approveNewRegistration(profileId: string, reason: string = '') {
  await verifyStaffAccess();
  const admin = createAdminClient();
  const safeReason = sanitizeString(reason);

  // Get profile to locate user_id
  ...
  if (profile.user_id) {
    updates.push(
      admin.from('users').update({ is_active: true }).eq('id', profile.user_id)
    );
  }
...
We are thrilled to inform you that your CenterKick registration and profile verification request have been officially approved!

Admin Message:
${safeReason || 'Welcome to the network! Your account is now fully active.'}

You can now log in and access all premium platform features, connect with scouts/agents, and manage your portfolio:
${SITE_URL}/login

Best regards,
The CenterKick Team`;

    await sendEmailNotification(email, "CenterKick - Account Verification Approved!", emailBody);
  }
...
export async function rejectNewRegistration(profileId: string, reason: string = '') {
  await verifyStaffAccess();
  const admin = createAdminClient();
  const safeReason = sanitizeString(reason);
  ...
  const updates = [
    admin.from('profiles').update({ status: 'rejected' }).eq('id', profileId)
  ];

  if (profile.user_id) {
    updates.push(
      admin.from('users').update({ is_active: false }).eq('id', profile.user_id)
    );
  }
...
We have reviewed your registration and profile verification request on CenterKick. Unfortunately, your request has been declined at this time.

Reason for Rejection:
${safeReason || 'The details provided could not be verified by our team.'}

If you have any questions or believe this is an error, please reach out to our administration team.

Best regards,
The CenterKick Team`;

    await sendEmailNotification(email, "CenterKick - Registration Status Update", emailBody);
  }
