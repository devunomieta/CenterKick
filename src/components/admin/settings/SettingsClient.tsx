  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative group">
        <div className={`w-full bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl p-6 transition-all flex flex-col items-center justify-center gap-3 ${isUploading ? 'opacity-50' : 'hover:border-gray-300 hover:bg-gray-100/50'}`}>
          {value ? (
            ...
            <div className="flex flex-col items-center gap-2 py-2">
               ...
               <p className="text-xs font-bold text-gray-900 tracking-wide">Click to upload {label.split(' ')[1]}</p>
            </div>
          )}
          <input 
            type="file" 
        ...
        {value && !isUploading && (
           <p className="text-xs font-bold text-gray-900 mt-2 truncate max-w-full px-2">{value}</p>
        )}
      </div>
    </div>
  );
...
export function SettingsClient({ initialSettings }: { initialSettings: Record<string, any> }) {
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-4 md:p-8 max-w-md w-full shadow-2xl border border-gray-100 animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-xl font-bold text-gray-900 tracking-tighter mb-2">Reset {activeSection}?</h3>
            <p className="text-xs text-gray-900 font-bold tracking-wide leading-relaxed mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button 
                ...
                className="flex-1 bg-red-500 text-white px-6 py-4 rounded-2xl font-bold text-xs tracking-wide transition-all hover:bg-red-600 shadow-xl shadow-red-100"
              >
                Confirm Reset
              </button>
              <button 
                onClick={() => setShowResetConfirm(false)}
                className="flex-1 bg-gray-100 text-gray-900 px-6 py-4 rounded-2xl font-bold text-xs tracking-wide transition-all hover:bg-gray-200"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button 
            ...
            disabled={isClearing}
            className="group min-w-0 md:min-w-full max-w-[200px] bg-white border border-gray-200 text-gray-900 px-6 py-4 rounded-2xl font-bold text-xs tracking-wide transition-all flex items-center justify-start gap-3 hover:bg-gray-50 hover:border-gray-100 disabled:opacity-50"
          >
 ...
 activeSection === section.title ? 'text-gray-900' : 'text-gray-900'
                  ...
                  <p className="text-xs font-bold text-gray-900 tracking-wide opacity-60">
                    {section.label}
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm min-h-[500px] flex flex-col">
            ...
            {activeSection === 'Global Configuration' && (
              <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between border-b border-gray-50 pb-6">
                   <div>
                      <h3 className="text-base font-bold text-gray-900 tracking-wide">Platform DNA</h3>
                      <p className="text-xs text-gray-900 font-bold mt-1">Core identity and accessibility rules</p>
                   </div>
                </div>

                <div className="grid grid-cols-1 gap-4 md:p-8">
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-gray-900 tracking-wide ml-1">Platform Name</label>
                      <input 
                        type="text" 
                 ...
                 <div className="mt-10 pt-10 border-t border-gray-50 flex justify-end">
                    <button 
                      onClick={handleSave}
                      disabled={isSaving}
                      className="bg-gray-900 text-white px-4 md:px-8 py-4 rounded-2xl font-bold text-xs tracking-wide transition-all hover:bg-black flex items-center gap-4 disabled:opacity-50 shadow-xl shadow-gray-100"
                    >
            ...
            {activeSection === 'Mail & SMTP Settings' && (
              <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between border-b border-gray-50 pb-6">
                   <div>
                      <h3 className="text-base font-bold text-gray-900 tracking-wide">Email Service (Resend)</h3>
                      <p className="text-xs text-gray-900 font-bold mt-1">Infrastructure for transactional emails</p>
                   </div>
                   <div className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl text-xs font-bold tracking-wide border border-indigo-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:p-8">
                   <div className="space-y-2 col-span-2">
                      <label className="text-xs font-bold text-gray-900 tracking-wide ml-1">Resend API Key</label>
                    ...
                    <div className="space-y-2 col-span-2">
                       <label className="text-xs font-bold text-gray-900 tracking-wide ml-1">From Email (Sender)</label>
                       <input 
                         type="text" 
                    ...
                    <div className="space-y-4 col-span-2 mt-4 p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                       <div className="flex items-center gap-2 mb-2">
                          ...
                          <h4 className="text-xs font-bold text-gray-700 tracking-wide">Connectivity Test</h4>
                       </div>
                       <div className="flex flex-col sm:flex-row gap-3">
                          <input 
                            type="email" 
                          ...
                          <button 
                            ...
                            className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold text-xs tracking-wide transition-all hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2"
                          >
                             {isTestingEmail ? <Loader2 className="w-3 h-3 animate-spin" /> : <Mail className="w-3 h-3" />}
                             Verify Connection
                          </button>
                       </div>
                       <p className="text-xs text-gray-900 font-bold tracking-wide ml-1">
            ...
            {activeSection === 'Security & Access' && (
              <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between border-b border-gray-50 pb-6">
                   <div>
                      <h3 className="text-base font-bold text-gray-900 tracking-wide">Hardened Perimeter</h3>
                      <p className="text-xs text-gray-900 font-bold mt-1">Advanced authentication policies</p>
                   </div>
                </div>
                   ...
                   {[
                     ...
                     <div key={item.key} className="flex items-center justify-between p-6 bg-gray-50 rounded-[2rem]">
                        <div className="space-y-1">
                           <span className="text-xs font-bold text-gray-800 tracking-wide">{item.label}</span>
                           <p className="text-xs font-bold text-gray-900 tracking-[0.1em]">Recommended for admin roles</p>
                        </div>
                        <button 
                          onClick={() => updateSetting(item.key, !settings[item.key])}
            ...
            {activeSection === 'Banners & Assets' && (
              <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between border-b border-gray-50 pb-6">
                   <div>
                      <h3 className="text-base font-bold text-gray-900 tracking-wide">Brand Assets</h3>
                      <p className="text-xs text-gray-900 font-bold mt-1">Global imagery and iconography</p>
                   </div>
                </div>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:p-8">
   <div className="space-y-6">
      <h4 className="text-xs font-bold text-gray-900 tracking-wide px-1">Site & Footer</h4>
      <ImageUpload 
   ...
   <div className="space-y-6">
      <h4 className="text-xs font-bold text-gray-900 tracking-wide px-1">Internal Portals</h4>
      <ImageUpload 
    ...
    />
            ...
            {activeSection === 'Social Links' && (
              <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center justify-between border-b border-gray-50 pb-6">
                   <div>
                      <h3 className="text-base font-bold text-gray-900 tracking-wide">Social Profiles</h3>
                      <p className="text-xs text-gray-900 font-bold mt-1">Manage platform social handles</p>
                   </div>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-2">
                     <label className="text-xs font-bold text-gray-400 tracking-wide ml-1">Facebook URL</label>
                     <input 
                   ...
                   <div className="space-y-2">
                     <label className="text-xs font-bold text-gray-400 tracking-wide ml-1">Instagram URL</label>
                     <input 
                       className="w-full bg-gray-50 border-none rounded-2xl p-4 text-xs font-bold focus:ring-2 focus:ring-[#b50a0a] transition-all text-black placeholder:text-gray-300"
                   ...
                   <div className="space-y-2">
                     <label className="text-xs font-bold text-gray-400 tracking-wide ml-1">X (Twitter) URL</label>
                     <input 
                       className="w-full bg-gray-50 border-none rounded-2xl p-4 text-xs font-bold focus:ring-2 focus:ring-[#b50a0a] transition-all text-black placeholder:text-gray-300"
                   ...
                   <div className="space-y-2">
                     <label className="text-xs font-bold text-gray-400 tracking-wide ml-1">YouTube URL</label>
                     <input 
                       className="w-full bg-gray-50 border-none rounded-2xl p-4 text-xs font-bold focus:ring-2 focus:ring-[#b50a0a] transition-all text-black placeholder:text-gray-300"
            ...
            <div className="mt-auto pt-10 border-t border-gray-50 flex flex-col md:flex-row items-center justify-between gap-4">
              <button 
                ...
                className="flex items-center gap-3 text-xs font-bold tracking-wide text-red-500 hover:text-red-700 transition-all hover:translate-x-1"
              >
                <Trash2 className="w-4 h-4" /> Reset {activeSection.split(' ')[0]} to Defaults
              </button>
               <div className="text-right">
                 <p className="text-xs font-bold text-gray-900 tracking-wide opacity-50">
